...
exports.sendMail = async function (message) {
  ...
  try {
    ...
    throw new Error(
      "Something is wrong with our mail service, please try again."
    );
  }
};
...
exports.sendVerificationEmail = async (option = {}) => {
  ...
  const secure = helper.generateRememberedToken({ email: option.to });
  // uses crypto to encrypt the jwt hash
  let encryptedLink = `${config.host}/account/verify/?secure=${helper.encrypt(
    secure
  )}`;
  const data = {
    userName: option.name || option.to,
    buttonText: "Verify Account",
    buttonLink: encryptedLink,
    header: true,
...
exports.sendResetPasswordMail = async (option = {}) => {
  try {
    ...
    let encryptedLink = `${
      config.host
    }/resetPassword/confirm?secure=${helper.encrypt(secure)}`;
    const data = {
      userName: option.name || option.to,
